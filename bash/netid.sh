...
# loop through the interface names returned expect the loopback interface

for infc in $(ip -o link show | awk -F': ' '{print $2}'); do
...
##### List of Interfaces in System expect loopback #####
-------------------------------------------------------
${interfaces[@]}
-------------------------------------------------------
EOF

####while loop for getting information of interface ####################
while [ $# -gt 0 ]; do
	case "$1" in
		-v | --verbose )
			verbose=yes
